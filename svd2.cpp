...
#define L_RATE 0.01
#define ERR_MARGIN L_RATE * 0.001
...
void train(double ** training_pointers, double ** feature_pointers) {
    ...
    for (i = 0; i < NUM_COMPONENTS; i++) {
        ...
        while (err > ERR_MARGIN || err < -ERR_MARGIN) {
            user = training_users[j];
            movie = training_movies[j];
            rating = training_ratings[j];
            ...
            tmp_feature = user_features[(int)user * NUM_COMPONENTS + i];
            user_features[(int)user * NUM_COMPONENTS + i] += err * movie_features[(int)movie * NUM_COMPONENTS + i];
            movie_features[(int)movie * NUM_COMPONENTS + i] += err * tmp_feature;
            j++;
            if (j == NUM_TRAINING) {
                j = 0;
            }
